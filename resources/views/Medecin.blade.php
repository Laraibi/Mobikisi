        ...
        <div class="card w-100" id="doctorsList">
            ...
            <div class="card-body">
                <ul class="users-list">
                    ...
                    <li class="">
                        <button type="button" class="btnDeleteMedecin text-red close d-none position-absolute"
                            aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
...
<script>
    ...
    $(document).ready(function () {
        $('.users-list li').mouseenter(function () {
        ...
        $('a.users-list-name').click(function () {
            $.ajax({
                ...
                success: (data) => {
                    ...
                    $('.modal .btn-primary').removeClass('btn-primary').addClass('btn-warning');
                    $('#inputFullName').val(data.fullName);
                    $('#inputSpecialite').val(data.Specialite);
                    $('#inputDOB').val(data.DateOfBirth);
                    $("input:radio[name=sexe][value=" + data.sexe + "]").prop('checked', true);
                    $('#imgMedecin').attr('src', '/storage/Images/Medecins_Photos/' + data.photo_path);
                    $('#addMedecinModal').modal('toggle');
                }
            });
        });
        $('.btnDeleteMedecin').click(function () {
            let MedecinId = $(this).parent().find('a.users-list-name').attr('data');
            // alert(MedecinId);
            $.ajax({
